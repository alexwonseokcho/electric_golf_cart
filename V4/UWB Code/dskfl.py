...
# Constants
sensor1_pos = np.array([0.25, 0])  # Position of sensor 1 (origin)
sensor2_pos = np.array([-0.25, 0])  # Position of sensor 2 (10 meters away)
...
def read_data():
    ...
    while(True):
        ...
        try:
            ...
            # estimated_position, particles = particle_filter(d_left, d_right, particles)
            # x, y = calculate_position(d_left, d_right)

            x_data = x
            y_data = y
            # x_data = estimated_position[0]
            # y_data = estimated_position[1]


            # f.write(f"{x} {y}\r")
            # f.flush()
            # print(f"{x} {y}")
            print(f'left: {d_left}, right: {d_right}, x: {x_data}, y: {y_data}')
        except:
            pass
        time.sleep(0.01)

...
//     return; // not supported
//   }
//   CanMsg msg = {.id = (unsigned int)ESP32Can.packetId(), .len = (uint8_t)packet_size};
//   ESP32Can.readBytes(msg.buffer, packet_size);
//   onCanMessage(msg);
// }
...
bool setupCan() {
  ...
  // ESP32Can.onReceive(receiveCallback); //might have to poll for received data every cycle instead of using a callback
  //library does not support callbacks
  return true;
}
...
void setup() {
  ...
  // while(true){
  //   CanFrame rxFrame;
  //   if(ESP32Can.readFrame(rxFrame, 700)) Serial.println("Received CAN message");
  //   else Serial.println("No CAN message received");
  // }
...
void loop() {
  //pump events equivalent
  // CanFrame tempFrame;
  // if(ESP32Can.readFrame(tempFrame, 0)){
  //   onCanMessage(tempFrame);
  // }

...
void IMUTask( void * ) { //task for getting IMU data
  ...
  while(true){
    if (bno08x.getSensorEvent(&sensorValue))
    {
      ...
      //also get the acceleration values
      double heading = -ypr.yaw;      
      double pitch = -(ypr.pitch - 48.6);
      if(heading < 0.0) heading += 360.0;
      // Serial.println("yaw pitch roll: " + String(heading) + " " + String(pitch) + " " + String(ypr.roll));
...
void quaternionToEulerRV(sh2_RotationVectorWAcc_t* rotational_vector, euler_t* ypr, bool degrees = false) {
  ...
  // wResult = rotational_vector->real;
  // xResult = rotational_vector->i;
  // yResult = rotational_vector->j;
  // zResult = rotational_vector->k;
